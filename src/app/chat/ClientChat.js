...
import { getMessages, sendMessage, initializeDirectConnect } from "@/app/actions/chat";
...
export default function ClientChat({ initialChats, currentUser, encryptedPrivateKey }) {
  ...
  // Replay-Attack-Safe Polling implementation & E2EE Decryption
  useEffect(() => {
    let interval;
    if (activeChatId && activeSharedSecret) {
      const fetchMsgs = async () => {
        try {
          ...
          // Decrypt payload and verify Anti-Masquerade Signature
          const decryptedMsgs = await Promise.all(msgs.map(async (msg) => {
            try {
              if (msg.sender.username === currentUser) {
                // If it's my own message, skip or rely on local memory?
                // Actually, I should just decrypt it. Since I sent it, I signed it with my own key.
                const myPubKeys = JSON.parse(chats.find(c => c.id === activeChatId).participants.find(p => p.user.username === currentUser).user.publicKey);
                msg.content = await CryptoEngine.verifyAndDecryptMessage(msg.content, activeSharedSecret, myPubKeys.sig);
                return msg;
              } else {
                const peerPubKeys = JSON.parse(msg.sender.publicKey);
                msg.content = await CryptoEngine.verifyAndDecryptMessage(msg.content, activeSharedSecret, peerPubKeys.sig);
                return msg;
              }
            } catch (decErr) {
          ...
          setMessages(decryptedMsgs);
           ...
           console.error("Secure fetch failure");
        }
      };
      
      fetchMsgs(); // Initial
      interval = setInterval(fetchMsgs, 3000); // Polling constraint
    } else {
      setMessages([]); // Clear messages when lock drops
    }
  ...
  async function handleSend(e) {
    ...
    // E2EE Encryption and Digital Signature
    try {
      const encryptedPayload = await CryptoEngine.encryptAndSignMessage(content, activeSharedSecret, privKeys.privSig);
      
      const newMsgRaw = await sendMessage(activeChatId, encryptedPayload);
      const newMsg = { ...newMsgRaw, content: content }; // Optimistic plaintext load
  ...
  async function handleNewConnection(e) {
    ...
    try {
      const newChat = await initializeDirectConnect(targetUser);
    ...
    return (
      <main style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
        <form onSubmit={handleUnlock} className="glass-panel" style={{ padding: "3rem", width: "100%", maxWidth: "450px", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          ...
          <p style={{ textAlign: "center", fontSize: "0.875rem", color: "var(--text-secondary)" }}>Your private identity keys are encrypted locally. Input passphrase to decrypt keys into volatile memory.</p>
          {unlockError && <div style={{ color: "#ff4d4d", fontSize: "0.875rem", border: "1px solid rgba(255, 77, 77, 0.3)", padding: "0.5rem", borderRadius: "4px", background: "rgba(255, 0, 0, 0.1)" }}>{unlockError}</div>}
          ...
          <button type="button" onClick={() => signOut()} style={{ background: "transparent", color: "#ff4d4d", border: "1px solid #ff4d4d" }}>TERMINATE SESSION</button>
        </form>
      </main>
    );
  }
    ...
    <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden" }}>
      ...
      <div className="glass-panel" style={{ width: "300px", borderRight: "1px solid var(--glass-border)", display: "flex", flexDirection: "column", borderRadius: "0" }}>
        <div style={{ padding: "1.5rem", borderBottom: "1px solid var(--glass-border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button onClick={() => signOut()} style={{ padding: "8px", fontSize: "0.75rem", border: "1px solid #ff4d4d", color: "#ff4d4d", background: "transparent" }}>LOCK NODE</button>
        </div>
          ...
          <>
            <div style={{ padding: "1.5rem", borderBottom: "1px solid var(--glass-border)", background: "rgba(4, 13, 20, 0.8)", backdropFilter: "blur(10px)" }}>
              <div style={{ fontSize: "0.875rem", color: "var(--accent-cyan)", display: "flex", justifyContent: "space-between" }}>
                ...
                {activeSharedSecret ? <span style={{ color: "#00e5ff" }}>[VERIFIED]</span> : <span style={{ color: "#ff4d4d" }}>[NEGOTIATING KEYS...]</span>}
              </div>
              ...
              {messages.map(msg => {
                ...
                return (
                  <div key={msg.id} style={{ 
                    ...
                    {!isMine && <div style={{ fontSize: "0.75rem", color: "var(--text-secondary)", marginBottom: "4px", display:"flex", alignItems:"center", gap:"8px" }}>
                      {msg.sender?.username} 
            ...
            <div style={{ padding: "1.5rem", borderTop: "1px solid var(--glass-border)", background: "rgba(4, 13, 20, 0.8)", backdropFilter: "blur(10px)" }}>
                <form onSubmit={handleSend} style={{ display: "flex", gap: "1rem" }}>
                  <button type="button" onClick={() => alert("Secure P2P Upload Bridge Offline")} style={{ padding: "12px", background: "transparent", border: "1px dashed var(--text-secondary)", color: "var(--text-secondary)" }}>ATTACH</button>
                  ...
                  <button type="submit" disabled={!activeSharedSecret} style={{ opacity: activeSharedSecret ? 1 : 0.5 }}>TRANSMIT</button>
                </form>
            </div>
          </>
        ) : (
